...
### Acceptance
Accepted terms are added to `glossary/glossary.md` and `glossary/glossary.json`, added to the relevant taxonomy file, referenced in `schema/terminology-ontology-lite.json`, and documented in `CHANGELOG.md` as a minor version increment.
...
This authority is exercised to protect the integrity and utility of the DSG framework as a conceptual reference for governance practice. It is not exercised to exclude legitimate governance research or commentary — researchers and practitioners may use, adapt, and build upon DSG terminology under the CC BY 4.0 license, with appropriate attribution.
